...
Elm.GotScales = Elm.GotScales || {};
Elm.GotScales.make = function (_elm) {
   "use strict";
   _elm.GotScales = _elm.GotScales || {};
   if (_elm.GotScales.values) return _elm.GotScales.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var renderNote = F3(function (address,classNames,note) {
      return A2($Html.a,_U.list([$Html$Attributes.$class(classNames),A2($Html$Events.onClick,address,note)]),_U.list([$Html.text(note)]));
   });
   var renderNotes = F3(function (address,classNames,notes) {    return A2($List.map,A2(renderNote,address,classNames),notes);});
   var constants = {naturalText: "♮ Naturals",sharpsAndFlatsText: "♯ Sharps ♭ Flats",showNaturals: "SHOW_NATURALS",showSharpsAndFlats: "SHOW_SHARPS_AND_FLATS"};
   var renderNoteType = F2(function (address,noteType) {
      var sharpsAndFlatsLink = _U.eq(noteType,constants.showSharpsAndFlats) ? A2($Html.span,
      _U.list([$Html$Attributes.$class("NoteType__toggle NoteType__toggle--active")]),
      _U.list([$Html.text(constants.sharpsAndFlatsText)])) : A2($Html.a,
      _U.list([$Html$Attributes.$class("NoteType__toggle pointer"),A2($Html$Events.onClick,address,constants.showSharpsAndFlats)]),
      _U.list([$Html.text(constants.sharpsAndFlatsText)]));
      var naturalLink = _U.eq(noteType,constants.showNaturals) ? A2($Html.span,
      _U.list([$Html$Attributes.$class("NoteType__toggle NoteType__toggle--active")]),
      _U.list([$Html.text(constants.naturalText)])) : A2($Html.a,
      _U.list([$Html$Attributes.$class("NoteType__toggle pointer"),A2($Html$Events.onClick,address,constants.showNaturals)]),
      _U.list([$Html.text(constants.naturalText)]));
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("NoteType")]),
      _U.list([naturalLink,A2($Html.span,_U.list([$Html$Attributes.$class("mh1")]),_U.list([$Html.text("•")])),sharpsAndFlatsLink]));
   });
   var renderNoteSelector = F3(function (address,notes,noteType) {
      return _U.eq(noteType,constants.showNaturals) ? A2($Html.div,
      _U.list([$Html$Attributes.$class("NoteSelector")]),
      A3(renderNotes,address,"NoteSelector__note NoteSelector__note--natural",notes)) : A2($Html.div,
      _U.list([$Html$Attributes.$class("NoteSelector")]),
      A3(renderNotes,address,"NoteSelector__note NoteSelector__note--sharpsAndFlats",notes));
   });
   return _elm.GotScales.values = {_op: _op
                                  ,constants: constants
                                  ,renderNoteType: renderNoteType
                                  ,renderNoteSelector: renderNoteSelector
...
Elm.Main.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $GotScales = Elm.GotScales.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var notesArray = Elm.Native.Port.make(_elm).inboundSignal("notesArray",
   "GotScales.Chord",
   function (v) {
   ...
   var noteMailbox = $Signal.mailbox("");
   var setNote = Elm.Native.Port.make(_elm).outboundSignal("setNote",function (v) {    return v;},noteMailbox.signal);
   var noteTypeMailbox = $Signal.mailbox("");
   var view = function (model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("GotScales")]),
      _U.list([A2($GotScales.renderNoteType,noteTypeMailbox.address,model.noteType)
              ,A3($GotScales.renderNoteSelector,noteMailbox.address,model.notesArray,model.noteType)]));
   };
   var setNoteType = Elm.Native.Port.make(_elm).outboundSignal("setNoteType",function (v) {    return v;},noteTypeMailbox.signal);
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "SetNoteType": return _U.update(model,{noteType: _p0._0});
         case "UpdateNotes": return _U.update(model,{notesArray: _p0._0});
         case "SetNote": return _U.update(model,{noteSelected: _p0._0});
         default: return model;}
   });
   var SetNote = function (a) {    return {ctor: "SetNote",_0: a};};
   var SetNoteType = function (a) {    return {ctor: "SetNoteType",_0: a};};
   var UpdateNotes = function (a) {    return {ctor: "UpdateNotes",_0: a};};
   var actions = $Signal.mergeMany(_U.list([A2($Signal.map,UpdateNotes,notesArray)
                                           ,A2($Signal.map,SetNoteType,noteTypeMailbox.signal)
                                           ,A2($Signal.map,SetNote,noteMailbox.signal)]));
   var NoOp = {ctor: "NoOp"};
   var initialModel = {noteType: $GotScales.constants.showNaturals,notesArray: _U.list([]),noteSelected: ""};
   var modelChanges = A3($Signal.foldp,update,initialModel,actions);
   var main = A2($Signal.map,view,modelChanges);
   var Model = F3(function (a,b,c) {    return {noteType: a,notesArray: b,noteSelected: c};});
   return _elm.Main.values = {_op: _op
                             ...
                             ,update: update
                             ,actions: actions
                             ,noteTypeMailbox: noteTypeMailbox
                             ,noteMailbox: noteMailbox
                             ,view: view
                             ,main: main};
};
